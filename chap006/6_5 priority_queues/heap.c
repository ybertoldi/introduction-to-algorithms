...
void max_heap_insert(heap *A, int val){
  ...
  A->size = A->size + 1;
  A->values[A->size - 1] = val;

  int i = A->size - 1;
  while (i > 0 && A->values[i] > A->values[find_parent(i)]) {
    int temp = A->values[i];
    A->values[i] = A->values[find_parent(i)];
    A->values[find_parent(i)] = temp;
    i = find_parent(i);
  }
}
